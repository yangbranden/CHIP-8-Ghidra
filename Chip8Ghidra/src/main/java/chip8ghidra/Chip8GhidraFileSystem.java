...
@FileSystemInfo(type = "fstypegoeshere", // ([a-z0-9]+ only)
		description = "File system description goes here", factory = Chip8GhidraFileSystem.MyFileSystemFactory.class)
public class Chip8GhidraFileSystem implements GFileSystem {
	 ...
	 */
	public Chip8GhidraFileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + Chip8GhidraFileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryByteProvider<Chip8GhidraFileSystem>,
			GFileSystemProbeByteProvider {

		@Override
		public Chip8GhidraFileSystem create(FSRLRoot targetFSRL,
				ByteProvider byteProvider, FileSystemService fsService, TaskMonitor monitor)
				throws IOException, CancelledException {

			Chip8GhidraFileSystem fs = new Chip8GhidraFileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
